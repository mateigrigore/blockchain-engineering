...
BLOCKCHAIN_COMMUNITY_ID = b"Lab3-Grp-Blockchain2"
...
DIFFICULTY = int(os.environ.get("DIFFICULTY", "22"))
...
class Chain:
    ...
    def validate_block(self, block: Block, parent: Block) -> bool:
        """Validate a block against its parent: hash, PoW, link, body commitment.

        Checks the four things every block must satisfy: header integrity,
        proof-of-work, parent link, and body commitment.

        NO transaction signature checks here: blocks travel between peers carrying
        only tx_hashes, never the full transactions, so the raw data a signature
        check needs isn't available at this point. Signatures are verified only
        once, when a tx is admitted (SubmitTx / TxGossip).
        """
...
class BlockchainCommunity(Community, PeerObserver):
    ...
    def _rebuild_mempool(self) -> None:
        ...
        on_chain = {th for b in self.chain.blocks for th in b.tx_hashes}
    ...
    def on_get_chain_height(self, peer: Peer, payload: GetChainHeight) -> None:
        tip = self.chain.tip()
        print(f"[chain] GetChainHeight request_id={payload.request_id} -> h={tip.height} {short(tip.block_hash)}")
    ...
    def on_get_block(self, peer: Peer, payload: GetBlock) -> None:
        if 0 <= payload.height <= self.chain.tip().height:
            print(f"[chain] GetBlock h={payload.height} -> {short(self.chain.blocks[payload.height].block_hash)}")
            self.ez_send(peer, self._block_payload(BlockResponse, self.chain.blocks[payload.height]))
    ...
    def on_new_block(self, peer: Peer, payload: NewBlock) -> None:
        ...
        if block.height == tip.height + 1 and block.prev_hash == tip.block_hash:
            ...
            if self.chain.validate_block(block, tip):
    ...
    def _maybe_swap_tip(self, block: Block) -> None:
        ...
        if tip.height < 1:
            return
        parent = self.chain.blocks[tip.height - 1]
        if not self.chain.validate_block(block, parent):
            return
    ...
    def on_get_full_chain(self, peer: Peer, payload: GetFullChain) -> None:
        ...
        for h in range(start, self.chain.tip().height + 1):
            self.ez_send(peer, self._block_payload(ChainBlock, self.chain.blocks[h]))
    ...
    def _try_adopt_synced_chain(self) -> None:
        ...
        current = self.chain.blocks
        better = (len(candidate) > len(current)) or (
            len(candidate) == len(current)
            and int.from_bytes(candidate[-1].block_hash, "big")
            < int.from_bytes(current[-1].block_hash, "big"))
        if not better:
            return

        self.chain.blocks = candidate
        self.mining_generation += 1
        self._rebuild_mempool()
    ...
    def _accept_local_block(self, block: Block) -> None:
        ...
        # validate the block fully before accepting and broadcasting, even though we
        # built it ourselves: the search only guarantees the hash and parent link.
        if not self.chain.validate_block(block, tip):
            return
        self.chain.append_block(block)
        self.mining_generation += 1
...
async def start_client():
    ...
    print(f"Node up on UDP port {PORT}, key={os.path.basename(KEY_PATH)}, "
          f"difficulty={DIFFICULTY}, community={short(BLOCKCHAIN_COMMUNITY_ID)}")
